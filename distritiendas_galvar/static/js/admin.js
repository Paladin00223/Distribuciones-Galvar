...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadRevenueChart = async () => {
        try {
            ...
            let apiUrl = '/api/admin/monthly_revenue';
            ...
            if (result.success) {
                                ...
                                tooltip: {
                                    callbacks: {
                                        label: function(context) {
                                            return `Ingresos: ${new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 }).format(context.parsed.y)}`;
                                        }
                                    }
                                }
                            }
                ...
                throw new Error(result.message || 'Error al cargar datos del gráfico.');
            }
        } catch (error) {
    ...
    const loadUsers = async (page = 1) => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/users?page=${page}&limit=${limit}&search=${encodeURIComponent(searchTerm)}`);
            if (!response.ok) {
            ...
            users.forEach(user => {
                ...
                row.innerHTML = `
                    ...
                    <td class="acciones">
                        <button class="btn-accion editar" data-type="user" data-id="${user.uid}">Editar</button>
                        ${deleteButtonHtml}
                    </td>
                `;
            });
            renderPaginationControls('usuarios-pagination', current_page, total_pages, loadUsers);
        } catch (error) {
    ...
    const loadOrders = async (page = 1) => {
        const statusFilter = document.getElementById('filtroEstado')?.value || 'todos';
        const searchTerm = document.getElementById('buscarPedido')?.value || '';
        const limit = 10; // Debe coincidir con el backend

        try {
            // Realiza la llamada a la API de Flask para obtener los pedidos
            const response = await fetch(`/api/admin/orders?status=${statusFilter}&page=${page}&limit=${limit}&search=${encodeURIComponent(searchTerm)}`);
            if (!response.ok) {
                throw new Error(`Error HTTP: ${response.status}`);
            }
             const { orders, total_pages, current_page } = await response.json();
            ...
            orders.forEach(order => {
                ...
                const productNames = order.items.map(item => `${item.nombre || 'Producto desconocido'} (x${item.cantidad})`).join('<br>');
            ...
            renderPaginationControls('pedidos-pagination', current_page, total_pages, loadOrders);
        } catch (error) {
    ...
    const loadProducts = async (page = 1) => {
        try {
            ...
            const data = await response.json();
            if (!data.products) throw new Error('Error al cargar productos');

            const productsTableBody = document.getElementById('productos-tabla');
            productsTableBody.innerHTML = '';
            const { products, total_pages, current_page } = data;

            if (products.length === 0) {
                productsTableBody.innerHTML = '<tr><td colspan="6" style="text-align: center;">No hay productos para mostrar.</td></tr>';
                return;
            }

            products.forEach(product => {
                const row = productsTableBody.insertRow();
                const imageUrl = product.imagen ? `/static/uploads/products/${product.imagen}` : '/static/images/placeholder.png';
                row.innerHTML = `
                    <td><img src="${imageUrl}" alt="${product.nombre}" style="width: 50px; height: 50px; object-fit: cover;"></td>
                    <td>${product.nombre}</td>
                    <td>${product.categoria}</td>
                    <td>$${product.precio.toFixed(2)}</td>
                    <td>${product.stock}</td>
                    <td class="acciones">
                        <button class="btn-accion editar" data-type="product" data-id="${product._id}">Editar</button>
                        <button class="btn-accion eliminar" data-type="product" data-id="${product._id}">Eliminar</button>
                    </td>
                `;
            });
            renderPaginationControls('productos-pagination', current_page, total_pages, loadProducts);
        } catch (error) {
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        try {
            ...
            if (response.ok && result.success) {
                ...
                throw new Error(result.message || 'Error desconocido al cambiar el estado.');
            }
        } catch (error) {
    ...
    const deleteProduct = async (productId) => {
        try {
            ...
            if (response.ok && result.success) {
                ...
                throw new Error(result.message || 'Error desconocido al eliminar el producto.');
            }
        } catch (error) {
    ...
    const renderPaginationControls = (containerId, currentPage, totalPages, loadFunction, context = 2) => {
        ...
        const createButton = (page, text = page, isDisabled = false, isActive = false) => {
        ...
        // Botón "Anterior"
        container.appendChild(createButton(currentPage - 1, 'Anterior', currentPage === 1));
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            const shouldShow = (i === 1) || (i === totalPages) || (i >= currentPage - context && i <= currentPage + context);
        ...
        // Botón "Siguiente"
        container.appendChild(createButton(currentPage + 1, 'Siguiente', currentPage === totalPages));
    };
    ...
    const closeOrderDetailsButton = document.querySelector('.order-details-close');
    const orderDetailsContent = document.getElementById('order-details-content');

    const productModal = document.getElementById('product-modal');
    const closeProductButtons = document.querySelectorAll('.product-close-button');
    ...
    const renderOrderDetails = (order) => {
        const itemsHtml = order.items.map(item => `
            <li><span>${item.nombre || 'Producto desconocido'} (x${item.cantidad})</span><strong>$${(item.precio_compra * item.cantidad).toFixed(2)}</strong></li>
        `).join('');
        ...
        const detailsHtml = `
            <div class="details-section">
                ...
                <ul>
                    <li><span>Nº Pedido:</span><strong>${order.order_uid}</strong></li>
                    <li><span>ID de Referencia:</span><strong>${order._id}</strong></li>
                    <li><span>Fecha:</span><strong>${new Date(order.order_date).toLocaleString('es-ES')}</strong></li>
                    <li><span>Estado:</span><strong>${order.status}</strong></li>
                    <li><span>Monto Total:</span><strong>$${order.total_amount.toFixed(2)}</strong></li>
                </ul>
            </div>
            <div class="details-section">
                <h4>Información del Cliente</h4>
                <ul>
                    <li><span>Nombre:</span><strong>${order.customer_info.nombre}</strong></li>
                    <li><span>Email:</span><strong>${order.customer_info.email || 'N/A'}</strong></li>
                    <li><span>Teléfono:</span><strong>${order.customer_info.phone || 'N/A'}</strong></li>
                </ul>
            </div>
            <div class="details-section" style="grid-column: 1 / -1;">
                <h4>Información de Envío</h4>
                <ul>
                    <li><span>Recibe:</span><strong>${order.shipping_info.name}</strong></li>
                    <li><span>Dirección:</span><strong>${order.shipping_info.address}</strong></li>
                    <li><span>Teléfono de Contacto:</span><strong>${order.shipping_info.phone}</strong></li>
                    <li><span>Email de Contacto:</span><strong>${order.shipping_info.email}</strong></li>
                </ul>
            </div>
            <div class="details-section" style="grid-column: 1 / -1;">
                <h4>Productos</h4>
    ...
    const openStatusModal = (orderId, currentStatus) => {
        ...
        const validStatuses = Array.from(statusSelect.options).map(opt => opt.value);
    ...
    const openProductModal = async (productId = null) => {
        ...
        try {
            ...
            if (productId) {
                ...
                if (result.success) {
                    ...
                    document.getElementById('image-preview').src = `/static/uploads/products/${p.imagen}`;
        ...
        } catch (error) { console.error('Error al abrir modal de producto:', error); }
    };
    ...
    window.addEventListener('click', (event) => {
        ...
        // Añadido para el modal de estado
    ...
    closeProductButtons.forEach(button => {
        button.addEventListener('click', () => productModal.style.display = 'none');
    });
    ...
    productForm.addEventListener('submit', async (event) => {
        ...
        const method = productId ? 'POST' : 'POST'; // Siempre POST para FormData
    ...
    if (exportCsvBtn) {
        exportCsvBtn.addEventListener('click', () => {
            const statusFilter = document.getElementById('filtroEstado')?.value || 'todos';
            const searchTerm = document.getElementById('buscarPedido')?.value || '';
            const exportUrl = `/api/admin/export_orders_csv?status=${statusFilter}&search=${encodeURIComponent(searchTerm)}`;
    ...
    document.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('btn-accion')) {
            ...
            if (target.classList.contains('editar')) { // Botón Editar
                if (target.dataset.type === 'user') {
                    openEditModal(id);
                } else if (target.dataset.type === 'product') {
                    openProductModal(id);
                }
            } else if (target.classList.contains('eliminar')) { // Botón Eliminar
                if (target.dataset.type === 'user') {
            ...
            } else if (target.classList.contains('cambiar-estado')) { // Botón Cambiar Estado
            ...
            } else if (target.classList.contains('ver')) { // Botón Ver Detalles
